...
class BuyNowAddressScreen extends StatefulWidget {
  final String totalAmount;

  const BuyNowAddressScreen({Key? key, required this.totalAmount}) : super(key: key);

  @override
  State<BuyNowAddressScreen> createState() => _BuyNowAddressScreenState();
}

class _BuyNowAddressScreenState extends State<BuyNowAddressScreen> {
  ...
  void onGooglePayResult(res) {
    ...
    addressServices.buyNow(context: context, address: addressToBeUsed, totalSum: double.parse(widget.totalAmount), isPaid: true);
  }
  ...
  void payPressed(String addressFromProvider) {
    ...
    if (isForm) {
      if (_addressFormKey.currentState!.validate()) {
        ...
        addressServices.saveUserAddress(context: context, address: addressToBeUsed);
        Navigator.push(context, MaterialPageRoute(builder: (_) => UPIPaymentScreen(totalAmount: widget.totalAmount,)));
      } else {
    ...
    if (isForm) {
      if (_addressFormKey.currentState!.validate()) {
        ...
        'Name: ${nameController.text} \n${housenoController.text}, \n${areaController.text}, \n${cityController.text}, \n${landMarkController.text} \nPin code: ${pinCodeController.text}'
            '\nContact No: ${phoneController.text}';
        addressServices.saveUserAddress(context: context, address: addressToBeUsed);
        addressServices.buyNow(context: context, address: addressToBeUsed, totalSum: double.parse("${widget.totalAmount}"), isPaid: false);
